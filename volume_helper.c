...
//saving for future use, potentiially
...
static int cmd_list(void){ //validated and checked
    ...
    for (int i = 0; i < count; i++){
        ...
        IAudioSessionControl_QueryInterface(ctrl, &IID_IAudioSessionControl2, (void**)&ctrl2);
...
static int cmd_set(DWORD pid, int vol){ // validated and odne
...
static int cmd_mute(DWORD pid, int mute){//validated and done
    ISimpleAudioVolume *sav = find_session_by_pid(pid);
    if (!sav) {fprintf(stderr, "ERROR:session_not_found\n"); return 1;}
...
int main(int argc, char *argv[]){/// validated and done
